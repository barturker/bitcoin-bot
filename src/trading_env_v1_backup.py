...
class BitcoinTradingEnv(gym.Env):
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[dict] = None) -> Tuple[np.ndarray, dict]:
        ...
        self.total_pnl = 0.0
        self.entry_market_quality = 0.0  # Track market quality at entry
    ...
    def _get_market_quality(self) -> float:
        """
        Assess current market quality for trading using multi-timeframe analysis.
        Returns value between -1 (bad) and 1 (good).

        Bad conditions (should NOT trade):
        - Low ADX (no trend, choppy)
        - Timeframes in conflict
        - No clear trend direction

        Good conditions (should trade):
        - Strong ADX (trending)
    ...
    def _close_position(self, reason: str = 'manual') -> float:
        ...
        self.trades.append({
            ...
            'reason': reason,
            'entry_market_quality': self.entry_market_quality  # Track for analysis
        })
        ...
        self.take_profit = 0.0
        self.position_duration = 0
        self.entry_market_quality = 0.0  # Reset entry quality
    ...
    def step(self, action) -> Tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if self.position is None:
            ...
            if action_type in ['long', 'short']:
                ...
                self.entry_market_quality = market_quality  # Track entry quality
            elif action_type == 'hold' and market_quality < 0:
                # REWARD: Not trading during bad conditions is GOOD
                # Small reward for patience
                reward += 0.05 * abs(market_quality)  # More reward for worse conditions
        else:
            # Have position
        ...
        # Step reward disabled - only reward on trade close
        # This prevents the model from learning to chase short-term noise
    ...
    def _calculate_reward(self, pnl: float, reason: str = 'manual') -> float:
        ...
        Risk-adjusted with market quality consideration.
        """
        ...
        # Good entry (high quality) + profit = bonus
        # Bad entry (low quality) + loss = extra penalty
        ...
        if pnl > 0:
            ...
            if entry_quality > 0:
                ...
                reward *= (1.0 + 0.3 * entry_quality)  # Up to 30% bonus
            # Bad setup + profit = lucky, no bonus
        else:
            # Losing trade
            if entry_quality < 0:
                # Bad setup + loss = should have known better
                reward *= (1.0 + 0.3 * abs(entry_quality))  # Extra penalty

        # Exit reason adjustments (simplified)
        if reason == 'take_profit':
            reward *= 1.2  # Clean exit at target

        elif reason == 'max_duration':
            reward *= 0.7  # Held too long

        # Stop loss is NEUTRAL - it's risk management working correctly
        # No bonus or penalty for SL hits
